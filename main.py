...
pygame.mixer.music.load(os.path.join(SOUNDS_FOLDER, "crs.mp3"))
...
for name, file in sound_files.items():
    full_path = os.path.join(SOUNDS_FOLDER, file)
    try:
        if os.path.exists(full_path):
            sound_effects[name] = pygame.mixer.Sound(full_path)
        else:
            print(f"Warning: Sound file {full_path} not found!")
...
original_bg_img = pygame.image.load(os.path.join(IMAGES_FOLDER, "background.png")).convert()
background_img = pygame.transform.scale(original_bg_img, (game.width, game.height))
original_bg_img_game = pygame.image.load(os.path.join(IMAGES_FOLDER, "background1.png")).convert()
background_img_game = pygame.transform.scale(original_bg_img_game, (game.width, game.height))
bg_offset_x = 0

# Загрузка спрайт-листа Волтера
walter_spritesheet_img = pygame.image.load(os.path.join(IMAGES_FOLDER, "walter_sprites.png")).convert_alpha()
...
try:
    explosion_sheet = pygame.image.load(os.path.join(IMAGES_FOLDER, "explosion_sprites.png")).convert_alpha()
...
class Klient():
    ...
    def create_sprites(self):
        try:
            # Основные спрайты
            self.k_body = pg.AnimatedSprite(os.path.join(IMAGES_FOLDER, 'NPC/body.png'), (38,65), (400, 420))
            
            # Голова (основа для позиционирования волос)
            head_path = os.path.join(IMAGES_FOLDER, f'NPC/heads/{self.skin}/{self.race}.png')
            default_head_path = os.path.join(IMAGES_FOLDER, f'NPC/heads/{self.skin}/human.png')
            self.k_head = self.get_fallback_sprite(head_path, default_head_path)
            
            # Одежда
            clothing_path = os.path.join(IMAGES_FOLDER, f'NPC/clothing/{self.gclothing}/{self.clothing}.png')
            self.k_clothing = pg.AnimatedSprite(clothing_path, (38,65), (self.k_body.x, self.k_body.y))
            
            # Уши
            ear_path = os.path.join(IMAGES_FOLDER, f'NPC/ears/{self.skin}/{self.race}.png')
            default_ear_path = os.path.join(IMAGES_FOLDER, f'NPC/ears/{self.skin}/human.png')
            self.k_ear = self.get_fallback_sprite(ear_path, default_ear_path)
            
            # Глаза
            eye_path = os.path.join(IMAGES_FOLDER, 'NPC/eyes/cock.png') if self.race == "cock" else os.path.join(IMAGES_FOLDER, 'NPC/eyes/human.png')
            self.k_eye = pg.AnimatedSprite(eye_path, (38,65), (self.k_body.x, self.k_body.y))
            
            # Нос
            if "goblin" in self.debuff_list:
                nose_path = os.path.join(IMAGES_FOLDER, 'NPC/noses/special/goblin.png')
                self.k_nose = pg.AnimatedSprite(nose_path, (38,65), (self.k_body.x, self.k_body.y))
            else:
                nose_path = os.path.join(IMAGES_FOLDER, f'NPC/noses/{self.skin}/{self.race}.png')
                default_nose_path = os.path.join(IMAGES_FOLDER, f'NPC/noses/{self.skin}/human.png')
            ...
            if self.race == "human":
                ...
                hair_path = os.path.join(IMAGES_FOLDER, f'NPC/hair/{self.gender}/{self.hair}.png')
                ...
                self.k_hair = None  # Если не человек, волос нет
    ...
    def create_fallback_sprites(self):
        ...
        try:
            self.k_body = pg.AnimatedSprite(os.path.join(IMAGES_FOLDER, 'NPC/body.png'), (38,65), (200, 400))
            self.k_head = pg.AnimatedSprite(os.path.join(IMAGES_FOLDER, f'NPC/heads/{self.skin}/human.png'), (38,65), (self.k_body.x, self.k_body.y))
            self.k_clothing = pg.AnimatedSprite(os.path.join(IMAGES_FOLDER, f'NPC/clothing/{self.gclothing}/{self.clothing}.png'), (38,65), (self.k_body.x, self.k_body.y))
            self.k_ear = pg.AnimatedSprite(os.path.join(IMAGES_FOLDER, f'NPC/ears/{self.skin}/human.png'), (38,65), (self.k_body.x, self.k_body.y))
            self.k_eye = pg.AnimatedSprite(os.path.join(IMAGES_FOLDER, 'NPC/eyes/human.png'), (38,65), (self.k_body.x, self.k_body.y))
            self.k_nose = pg.AnimatedSprite(os.path.join(IMAGES_FOLDER, f'NPC/noses/{self.skin}/human.png'), (38,65), (self.k_body.x, self.k_body.y))
            
            if self.race == "human":
                # Используем сохраненный тип прически
                self.k_hair = pg.AnimatedSprite(
                    os.path.join(IMAGES_FOLDER, f'NPC/hair/{self.gender}/{self.hair}.png'), 
                    (38,65), 
    ...
    def draw(self, surface):
        ...
        if self.race != "cock" and self.k_ear:
...
menu_title = pg.Text(game.width // 2, 120, "Walter's Bar", size=80, color=COLOR_TEXT_MENU, font_path= os.path.join("Font", "Font.ttf"))
...
def effect_hryu_hryu():
    if not "cock" in current_client.debuff_list and not "ram" in current_client.debuff_list and not "pig" in current_client.debuff_list and not "goblin" in current_client.debuff_list:
        ...
        if 'pig' in sound_effects:
            sound_effects['pig'].play()
...
def effect_bigus_de_nous():
    if not "bignose" in current_client.debuff_list:
        ...
        if 'cock' in sound_effects:  # Можно заменить на подходящий звук, если есть
            sound_effects['cock'].play()
...
def effect_bigus_de_glazus():
    if not "bigeye" in current_client.debuff_list:
        ...
        if 'cock' in sound_effects:  # Замените на нужный звук, если есть
            sound_effects['cock'].play()
...
def effect_de_greatus():
    if not "red" in current_client.debuff_list and not "green" in current_client.debuff_list and not "blue" in current_client.debuff_list and not "goblin" in current_client.debuff_list and not "black" in current_client.debuff_list:
        ...
        if 'ram' in sound_effects:  # Замените на подходящий звук, если есть
            sound_effects['ram'].play()
...
def effect_hop_hop_goblin():
    if not "red" in current_client.debuff_list and not "blue" in current_client.debuff_list and not "goblin" in current_client.debuff_list and not "black" in current_client.debuff_list:
        ...
        if 'pig' in sound_effects:  # Замените на нужный звук, если есть
            sound_effects['pig'].play()
...
def play_pouring_with_effect(effect_callback):
    ...
    if not walter.is_playing:
        ...
        # Применяем эффект сразу при наливании
...
def on_pouring_finished():
    ...
    # Здесь ничего не делаем с эффектом — он уже применён

def on_throw_bottle_finished():
    print("Анимация броска бутылки завершена")
    # Просто воспроизводим звук броска
    # Звук эффекта убираем, так как эффект уже применён
...
def throw_bottle(mouse_button=1):
    global animation_state, last_effect_callback
    if mouse_button == 1:  # Левая кнопка - бросить
        if len(current_client.debuff_list) > 0:
            if not walter.is_playing:
                animation_state = "throw_bottle"
                last_effect_callback = None  # Эффект уже применён при наливании, сбрасываем
            ...
            print("Нет эффектов для применения!")
    elif mouse_button == 3:  # Правая кнопка - сбросить эффекты
        if not walter.is_playing:
            current_client.debuff_list.clear()
            current_client.apply_changes()
            print("Эффекты сброшены")
...
def bottle_button_callback(event):
    # Обработка кнопки бутылки с учетом кнопки мыши
    if event.type == pygame.MOUSEBUTTONDOWN:
        if event.button == 1:  # Левая кнопка
            throw_bottle(mouse_button=1)
        elif event.button == 3:  # Правая кнопка
            throw_bottle(mouse_button=3)

bottle_button = button_manager.add_button(
    state="playing",
    x=560,
    y=450,
    width=45,
...
# Переназначаем callback для обработки разных кнопок мыши
def bottle_button_event_handler(event):
    if event.type in (pygame.MOUSEBUTTONDOWN, pygame.MOUSEBUTTONUP):
        if bottle_button.rect.collidepoint(event.pos):
            bottle_button_callback(event)

bottle_button.handle_event = bottle_button_event_handler
...
class GraphicsQualitySelector:
    ...
    def draw(self, surface):
        ...
        for i, rect in enumerate(self.option_rects):
            ...
            # base_color - кортеж с прозрачностью, поэтому берем первые 3 компоненты для draw.rect
    ...
    if game_state == "playing":
        for btn in button_manager.invisible_buttons:
            if btn.rect.collidepoint(mouse_pos):
                # Индекс кнопки в invisible_buttons (только невидимые)
                idx = button_manager.invisible_buttons.index(btn)
                # Подсказки начинаются с третьей кнопки (сдвиг 2) в invisible_buttons
                # Но у нас первая невидимая - бутылка, вторая - динамит, далее эффекты
                # Проверим и сдвинем индекс на 2 для эффектов
                effect_idx = idx - 2
                if 0 <= effect_idx < len(effects_tooltips):
                    name, prop = effects_tooltips[effect_idx]
...
def handle_event(event):
    ...
    # Для невидимых кнопок с особой обработкой (бутылка), мы вызываем их отдельно
    # Поэтому здесь вызываем для всех, кроме бутылки
    for btn in button_manager.buttons[game_state]:
        btn.handle_event(event)
    for btn in button_manager.invisible_buttons:
        if btn is bottle_button:
            bottle_button.handle_event(event)
        else:
            btn.handle_event(event)
